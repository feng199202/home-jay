						...
						<table cellspacing='0' cellpadding='0' width="100%" border='0'
							...
							<s:iterator value="admins" var="existAdmin">
								<s:if test="role.roleId !=1">
									<tr>
										<td><s:property value="adminId" />
										</td>
										<td><s:property value="realName" />
										</td>
										<td><s:property value="accountName" />
										</td>
										<td><s:property value="adminPhone" />
										</td>
										<td><s:property value="adminEmail" />
										</td>
										<td><s:date name="enrollDate" format="yyyy-MM-dd" /></td>
										<td><s:property value="role.roleName" /></td>
										<td><a onclick="return window.confirm('确定要删除?')"
											href="${pageContext.request.contextPath}/admin/manageAdminAction!deleteAdmin?admin.adminId=<s:property value="adminId"/>">删除</a>
										</td>
